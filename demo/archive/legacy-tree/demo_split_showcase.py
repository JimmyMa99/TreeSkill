#!/usr/bin/env python3
"""
展示树拆分能力 Demo - 生成可见的子skill文件夹

策略:
1. 基础优化 (Round 1)
2. 自动拆分 (Round 2) - 生成子skill
3. 跳过剪枝 - 保留所有子skill
4. 保存到文件系统
...
def main():
    """主流程"""
    logger.info("\n" + "="*60)
    logger.info("🌳 展示树拆分能力 Demo")
    logger.info("="*60)
    logger.info("展示: 优化 → 拆分 → 保存（跳过剪枝以保留子skill）")
...
"""
    ...
    tree = SkillTree(
        ...
        base_path=Path("demo-split-showcase/"),
    )
    ...
    # ========================================================================
    logger.info(f"\n{'='*60}")
    logger.info(f"💾 保存skill树（跳过剪枝以保留子skill）")
    logger.info(f"{'='*60}")

    # 保存最终结果
    output_path = Path("demo-split-showcase/")
    tree_round2.save(output_path)

    logger.info(f"\n✅ Skill树已保存到: {output_path}")

    # 显示树结构
    logger.info(f"\n🌲 最终的树结构:")
    logger.info(f"\n{tree_round2.list_tree()}")
    ...
    if tree_round2.root.children:
        logger.info(f"\n✨ 成功生成 {len(tree_round2.root.children)} 个子skill文件夹:")
        for child_name, child_node in tree_round2.root.children.items():
            child_dir = output_path / child_name
            if child_dir.exists():
                logger.info(f"   ✅ {child_name}/")
            else:
                logger.info(f"   ❌ {child_name}/ (未保存)")
